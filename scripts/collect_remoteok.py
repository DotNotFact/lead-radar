"""Разовый прогон коллектора RemoteOK.
Запуск: python -m scripts.collect_remoteok
"""
...
logger = logging.getLogger("lead_radar.scripts.collect_remoteok")


async def collect_and_store(
    settings: Settings, sources_config: SourcesConfig, keywords_config: KeywordsConfig
) -> dict[str, int]:
    config = sources_config.sources.get("remoteok")
    ...
    collector = RemoteOkCollector(
        queries=queries, contact_email=settings.contact_email, poll_interval=config.poll_interval
    )
